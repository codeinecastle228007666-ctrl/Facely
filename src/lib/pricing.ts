...
export const PRICES: Record<Currency, Record<TierId, number>> = {
  ...
  // 2026-06-26 — Stars-цены слегка пониже (-12…-20%) чтобы monthly
  // был frictionless для юзера (× Telegram cuts только ~30% → юзер
  // платит меньше чем RUB, но мы всё равно в плюсе). 20% pack-discount
  // сохранён: 280/(5×70)=0.80.
  XTR: {
    single: 70,      // 70 ⭐  (-12.5%)
    pack5: 280,      // 280 ⭐ (-12.5%, 20% off от 5×70=350)
    monthly: 1200,   // 1 200 ⭐/мес (-20%)
  },
};
