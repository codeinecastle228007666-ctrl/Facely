...
- **Чат с AI-косметологом** — задавайте вопросы по уходу через Groq API (3 бесплатных вопроса, далее платно)
...
- **Инвентарь средств** — добавляйте косметику по ссылке, фото состава или вручную. AI-анализ ингредиентов через Groq (безопасность, ключевые компоненты, совместимость с типом кожи)
...
4. Примените схему базы данных:
   ...
   npx prisma db seed
   ```
   Если `prisma db push` недоступен (нет прямого доступа к БД), выполните SQL-миграции вручную через Supabase SQL Editor. Все SQL-файлы лежат в `prisma/migrations/`.
...
├── components/
│   ├── dashboard/          # UserProfile, BalanceCard, StreakCard, Onboarding, SkinDiary, AchievementsModal
│   ├── effects/            # ResultModal, ConfettiEffect
│   ├── history/            # AnalysisCard
│   ├── inventory/          # InventoryPanel, AddProductModal
│   ├── purchase/           # PurchaseModal
│   ├── referral/           # ReferralStats
│   └── ui/                 # TabBar, Icons, ProgressBar, Skeleton, OfflineBanner
...
| `GROQ_BASE_URL` | Базовый URL Groq API (опционально) |
